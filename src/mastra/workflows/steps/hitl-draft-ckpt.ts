...
const hitlDraftCheckpoint = createStep({
    id: "hitl-draft-checkpoint",
    ...
    execute: async ({ inputData, mastra, resumeData, suspend }) => {
        ...
        if (!resumeData) {
            return await suspend({
                suspendResponse: "Are you happy with any of the generated drafts? If not, please provide feedback: " + agentResponse
            });
        }
        ...
        const newResponse = await agent.generate(
            `Refine the generated draft based on the user input: ${userInput}`,
            {
                memory: {
                    thread: threadId,
                    resource: resourceId
